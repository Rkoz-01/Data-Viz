...
ggplot(mtcars, aes(x=mpg)) + geom_histogram(binwidth=1) #all cars that fall within 1 mpg
```
...
```{r, eval=FALSE}
ggplot(mtcars, aes(x=factor(cyl))) + geom_histogram(binwidth = 1) #wont work if cyl is a factor. remove it to get a histogram
...
mtcars_summary <- mtcars %>% 
  group_by(cyl) %>%  # Group by the number of cylinders and generatre summary statistics
  summarise(
    mpg_mean = mean(mpg, na.rm = TRUE),  # mpg mean, na.rm removes "na" values
...
ggplot(data = mtcars_summary, aes(x = factor(cyl), y = qsec_mean)) + 
  geom_bar(stat = "identity") # if you speciofy the Y data, you need to specify the identity, otherwise you will get counts
```
...
Dont make bar plots of standard error
...
ggplot(data = mtcars, aes(x = factor(cyl), y = qsec, color = factor(cyl))) +
  geom_point(size = 5) +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", #add in errorbars
        color = "black", width = 0.2) +
  stat_summary(fun = mean, geom = "point", #add in the mean
    size = 5, color = "black") 
...
```{r, message=FALSE}
# Set our theme to classic, this is last for you R session
...
ggplot has pch argument that defines the point symbol
...
ggplot(data = mtcars, aes(x = factor(cyl), y = qsec)) +
  geom_point(size = 5, pch = 21, color = "black", aes(fill = factor(cyl))) + #move aes fill to the geom_point to get a better ledgend
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar",
        color = "black", width = 0.2) +
  stat_summary(fun = mean, geom = "point",
    size = 5, color = "black") 
...
# you can look up color hex codes on line and then use them in R
...
ggplot(data = mtcars, aes(x = factor(cyl), y = qsec)) +
  ...
  stat_summary(fun = mean, geom = "point",
    size = 5, color = "black") +
  scale_fill_manual(values = c("4"= "#0074D9","6"= "#01FF70", "8"= "#FF4136")) 
...
ggplot(data = mtcars, aes(x = factor(cyl), y = qsec)) +
  geom_point(position = position_jitter(width = 0.15),   # Slightly offset points
...
ggplot(data = mtcars, aes(x = factor(cyl), y = qsec)) +
  ...
  scale_x_discrete(labels = c("4" = "Slowest", 
                              "6" = "Still Slow", 
                              "8" = "Fast but Broke")) +    # Custom x axis labels
  scale_y_continuous(limits = c(12, 24), breaks = (c(12,18,24)))+  
   labs(
    title = "Quarter Mile speed",
    x = "",
    y = "seconds",
    fill = "Engine Size")+
 theme(
...
data <- gapminder %>% 
  filter(year=="2007") %>% #only the 2007 data
  dplyr::select(-year)# remove the column "year"
...
# Create plot here THIS IS BROKEN! GET THE REAL CODE
